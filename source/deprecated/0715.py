...
class convNet():
    ...
    def __init__(self):
        ...
        # self.opts1 = tf.train.RMSPropOptimizer(learning_rate, decay=0.9, momentum=0., epsilon=1e-10)
        # self.gradient1 = self.opts1.compute_gradients(self.cost1)
        #
        # self.step1 = self.opts1.apply_gradients(self.gradient1)

        # self.learning_step1 = tf.train.RMSPropOptimizer(learning_rate, decay=0.9, momentum=0., epsilon=1e-10).minimize(
        #     self.cost1)
...
def Scale(img):
    ...
    imgResize = img.reshape([210, 160], order=0) # from left->right,up->down

    # cv2.imshow("Imagetest", imgResize)
    # cv2.waitKey(100)
...
Value1 = np.zeros([batchSize,n_actions])

def getValue1(net,State1):
    global Value1
    global sess
    Value1 = sess.run(net.y, feed_dict={net.x: State1})
...
while frameCount is not maxFrame:
    ...
    while not ale.game_over():
        ...
        # temp = np.reshape(memory[memoryCount, 0:n_senses], [84, 84])
        score += reward
        ...
        if frameCount >= startLearningFrame - 1:
            ...
            # State0 = np.reshape(State0,[32,84,84])
            State1 = memory[index +1, 0:n_senses]
            # State1 = np.reshape(State1,[32,84,84])
