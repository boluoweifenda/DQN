...
logPath = "log/onlineCNN0710.txt"
modelPath = "modelRMS/"
dataPath = "dataRMS/"
frameSkip = 4
maxGeneration = 500
memorySize = 500000
memoryFreshSize = 10000
...
memoryFresh = np.zeros([memoryFreshSize,n_senses + n_actions + 1],dtype= 'uint8')
...
print '\ninit data'
t0 = time.time()
for frameCount in xrange(memorySize):

    if ale.game_over():
        print 'Episode: %03d Score: %02d Rom uasge: %02d%%' % (episode, scoreCount , 100. * frameCount / (memorySize) )
        episode += 1
        scoreSum += scoreCount
        scoreCount = 0
        ale.reset_game()
    ...
    memory[frameCount][0:5904] = imgBinarySequence # save states

    actionIndex = randrange(len(legal_actions)) # get action

    memory[frameCount][5904:5908] = 0
    memory[frameCount][5904 + actionIndex] = 1 # save action
    reward = ale.act(legal_actions[actionIndex])  # reward
    memory[frameCount][5908] = reward
    scoreCount += reward
...
for generation in xrange(loadModel+1,maxGeneration):
    ...
    t0 = time.time()
    for frameCount in xrange(memoryFreshSize):

        if ale.game_over():
            print 'Episode: %03d Score: %02d Rom uasge: %02d%%' % (episode, scoreCount , 100. * frameCount / memoryFreshSize)
            episode += 1
            scoreSum += scoreCount
            scoreCount = 0
            ale.reset_game()
        ...
        imgScale.dtype = 'uint8'
        imgBinarySequence = imgScale.reshape([5904],order = 0)
        memoryFresh[frameCount][0:5904] = imgBinarySequence # save states
        ...
        memoryFresh[frameCount][5904:5908] = 0
        memoryFresh[frameCount][5904 + actionIndex] = 1 # save action
        reward = ale.act(legal_actions[actionIndex])  # reward
        memoryFresh[frameCount][5908] = reward
        scoreCount += reward
    ...
    memory[0:(memorySize-memoryFreshSize)] = memory[memoryFreshSize:memorySize]
    memory[(memorySize-memoryFreshSize):memorySize] = memoryFresh[:]

    if saveData is True:
        np.save(dataPath + mark,memory)
        print 'data saved'

    # print 'Coverting data... '
    [State, Action, Reward] = Experiences2SAR(memory, windowSize=temporal_window, onehot=False)
    num_batch = (memoryFresh.shape[0] - 1) / batchSize
