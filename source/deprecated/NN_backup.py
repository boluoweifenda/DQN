...
class NN:
    ...
    def __init__(self, opt , trainable):
        ...
        # self.b_conv1 = \
        #     tf.get_variable(
        #         name='b_Conv1',shape=[32],trainable= trainable,dtype = self.dataType,
        #         initializer= self.initializer
        #     )
        # self.b_conv1 = tf.Variable(tf.constant(0.0,dtype=tf.float32,shape=[32]),trainable=self.trainable,
        #                            dtype = self.dataType)
        ...
        # self.b_conv2 = \
        #     tf.get_variable(
        #         name='b_Conv2', shape=[64], trainable=trainable, dtype=self.dataType,
        #         initializer=self.initializer
        #     )
        # self.b_conv2 = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[64]), trainable=self.trainable,
        #                            dtype=self.dataType)
        ...
        # self.b_conv3 = \
        #     tf.get_variable(
        #         name='b_Conv3', shape=[64], trainable=trainable, dtype=self.dataType,
        #         initializer=self.initializer
        #     )
        # self.b_conv3 = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[64]), trainable=self.trainable,
        #                            dtype=self.dataType)
        ...
        if self.RLType == 'Q-learning' or self.RLType == 'SARSA':
            self.y = tf.matmul(self.h_fc1, self.W_fc2 )
            self.y_acted = tf.reduce_sum(self.y * self.action_one_hot, reduction_indices=1)

            # if self.RLType == 'Q-learning':
            #     self.z1 = tf.reduce_max(self.z, reduction_indices=[1], keep_dims=False)
            # if self.RLType == 'SARSA':
            #     self.z1 =
        ...
        # elif self.RLType == 'Policy Gradient':
        #     self.y = tf.nn.softmax(tf.nn.sigmoid(tf.matmul(self.h_fc1, self.W_fc2)))
        #     # a = self.action_one_hot * tf.log(self.y)
        #     self.cost = -tf.reduce_sum(self.action_one_hot * tf.log(self.y))
        ...
        if self.trainable:
            ...
            # self.ops = tf.train.GradientDescentOptimizer(
            #             self.learningRate
            #             )
            # self.gradient = self.ops.compute_gradients(self.cost)
    #
