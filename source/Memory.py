...
class Memory:
    ...
    def __init__(self, opt):
        ...
        self.State = np.zeros([self.memorySize, self.height, self.width], dtype=np.uint8)
        self.History = np.zeros([self.historyLength, self.height, self.width], dtype=np.uint8)

        self.State0 = np.zeros([self.batchSize, self.historyLength, self.height, self.width], dtype=np.uint8)
        self.State1 = np.zeros([self.batchSize, self.historyLength, self.height, self.width], dtype=np.uint8)

        # SARSA
        self.Action1 = np.zeros(self.batchSize, dtype=np.uint8)
    ...
    def getBatch(self,batchSize, historyLength):
        ...
        while len(indexes) < batchSize:
            ...
            self.Action1[len(indexes)] = self.Action[index + 1]
        ...
        return self.State0, actions, rewards, self.State1,  terminals, self.Action1
